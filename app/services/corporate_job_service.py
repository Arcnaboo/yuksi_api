...
async def corporate_create_job(data: Dict[str, Any], corporate_id: str) -> Tuple[bool, str | None]:
    ...
    try:
        ...
        INSERT INTO admin_jobs (
            ...
            created_by_admin, restaurant_id, dealer_id, corporate_id,
...
# --- READ (liste) ---
async def corporate_get_jobs(corporate_id: str, limit: int = 50, offset: int = 0, delivery_type: str | None = None) -> Tuple[bool, List[Dict[str, Any]] | str]:
    """Kurumsal kullanıcı tarafından yük listesini getirir (sadece o kurumsal kullanıcının yükleri)"""
    try:
        filters = []
        params = [corporate_id]
        i = 2

        # Sadece bu kurumsal kullanıcının yüklerini getir
        filters.append("corporate_id = $1")
...
async def corporate_update_job(job_id: str, corporate_id: str, fields: Dict[str, Any]) -> Tuple[bool, Optional[str]]:
    ...
    try:
        ...
        params.extend([job_id, corporate_id])

        query = f"""
            UPDATE admin_jobs
            SET {', '.join(update_fields)}
            WHERE id = ${i} AND corporate_id = ${i + 1};
        """
...
# --- DELETE ---
async def corporate_delete_job(job_id: str, corporate_id: str) -> Tuple[bool, Optional[str]]:
    """Kurumsal kullanıcı tarafından yük silme servisi (sadece kendi yüklerini silebilir)"""
    try:
        # Önce yükün bu kurumsal kullanıcının olup olmadığını kontrol et
        check = await fetch_one(
            "SELECT id FROM admin_jobs WHERE id = $1 AND corporate_id = $2;",
        ...
        result = await execute("DELETE FROM admin_jobs WHERE id = $1 AND corporate_id = $2;", job_id, corporate_id)
        if result.endswith(" 0"):
