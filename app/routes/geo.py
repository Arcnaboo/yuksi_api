...
def get_countries(q: Optional[str] = Query(None, description="İsme göre arama"),
                  limit: int = Query(50, ge=1, le=200),
                  offset: int = Query(0, ge=0)):
    return list_countries(q=q, limit=limit, offset=offset)

@router.get("/states", response_model=List[StateOut], summary="Bir ülkenin eyalet/illerini listele")
def get_states(country_id: int = Query(..., description="Country ID"),
    ...
    return list_states_by_country(country_id, q=q, limit=limit, offset=offset)

@router.get("/cities", response_model=List[CityOut], summary="Bir ilin şehirlerini/ilçelerini listele")
def get_cities(state_id: int = Query(..., description="State ID"),
               q: Optional[str] = Query(None),
    ...
    return list_cities_by_state(state_id, q=q, limit=limit, offset=offset)
