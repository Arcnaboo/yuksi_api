...
class UserJobCreate(BaseModel):
    """Bireysel kullanıcı tarafından yük oluşturma modeli"""
    ...
    # ✅ Araç seçimi (3 farklı yöntem - en az biri dolu olmalı)
...
class UserJobListFilter(BaseModel):
    """Bireysel kullanıcı job listeleme filtre modeli"""
    limit: int = Field(50, ge=1, le=200)
    offset: int = Field(0, ge=0)
    deliveryType: Optional[Literal["immediate", "scheduled"]] = None


class UserJobUpdateReq(BaseModel):
    """Bireysel kullanıcı job güncelleme modeli"""
    deliveryType: Optional[Literal["immediate", "scheduled"]] = None
